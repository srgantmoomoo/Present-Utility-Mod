...
public class MainMenu extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for(String name : buttons) {
			...
			float y = (height/buttons.length) + (height/buttons.length) + count * 30;
			...
			this.drawCenteredString(mc.fontRendererObj, name, (width/buttons.length) + (width/buttons.length) + (width/buttons.length)/2f, (height/buttons.length) + (height/buttons.length) + count * 30, hovered ? 0x98700000 : -1);
			count++;
		}
